function mirrorDrawTool() {
	...
	this.icon = "assets/toolIcons/mirrorDraw.jpg";
    
    var show = true;
    //set the canvas details to a veriable name
	...
	this.draw = function() {
        ...
        //helper function for unhiding the erasertool
        hide(show);
		//display the last save state of pixels
		updatePixels();

		//do the drawing if the mouse is pressed
		if (mouseIsPressed && !useEraser) {
		...
		push();
        //hides the mirror line when using the eraser and restores it when not.
        if(!useEraser)
        {
            strokeWeight(3);
            stroke("red");
	...
	this.populateOptions = function() {
		select(".options").html(
			"<p>Tool Options</p><button id='directionButton'>Mirror Horizontal</button>");
		// 	//click handler
		select("#directionButton").mouseClicked(function() {
			...
			if (self.axis == "x") {
				...
				button.html('Mirror Vertical');
			} else {
				self.axis = "x";
				self.lineOfSymmetry = width / 2;
				button.html('Mirror Horizontal');
			}
		});
	};
}